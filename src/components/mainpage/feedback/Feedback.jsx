// Импорт стиля
import '../../../styles/feedback.css'
// Импорт хуков
import { useState } from 'react'
import { useWindowWidth } from '../../../utils'
// Импорт компонента
import FeedbackItem from "./FeedbackItem"

// Список отзывово 
const listFeedback = [
    {feedback: 'Я заказала последний iPhone из США через ваш сервис, и доставка прошла быстро и без каких‑либо проблем. Гарантии качества и надёжная упаковка сделали мой опыт покупки невероятно позитивным', author: 'Анна'},
    {feedback: 'Я уже несколько лет заказываю дизайнерскую одежду из Франции через ваш сервис, и он никогда меня не подводил. Важно иметь надёжную компанию, которая позаботится о вашем заказе и предоставит все необходимые документы для ввоза в Россию', author: 'Алексей'},
    {feedback: 'Отзыв Беллы', author: 'Белла'},
    {feedback: 'Отзыв Бориса', author: 'Борис'},
    {feedback: 'Отзыв Владиславы', author: 'Владислава'}
]

// Компонент отзывов
const Feedback = () => {
    const [currentPage, setCurrentPage] = useState(0)

    // Предыдущий отзыв
    const onClickPrev = () => {
        setCurrentPage(prev => (prev - 1 + listFeedback.length) % listFeedback.length)
    }

    // Следующий отзыв
    const onClickNext = () => {
        setCurrentPage(prev => (prev + 1 + listFeedback.length) % listFeedback.length)
    }

    return (
        <div className="feedback">
            <h2 className='xl-3 w-black'>Отзывы</h2>
            <div className="feedback-box">
                <svg className="arrow left" width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg" tabIndex={0} onClick={onClickPrev}>
                    <path d="M18.87 27.87C18.5969 28.1553 18.3828 28.4917 18.24 28.86C17.9399 29.5904 17.9399 30.4096 18.24 31.14C18.3828 31.5082 18.5969 31.8447 18.87 32.13L27.87 41.13C28.4349 41.6949 29.2011 42.0123 30 42.0123C30.7989 42.0123 31.5651 41.6949 32.13 41.13C32.6949 40.5651 33.0123 39.7989 33.0123 39C33.0123 38.2011 32.6949 37.4349 32.13 36.87L28.23 33H39C39.7956 33 40.5587 32.6839 41.1213 32.1213C41.6839 31.5587 42 30.7956 42 30C42 29.2043 41.6839 28.4413 41.1213 27.8787C40.5587 27.3161 39.7956 27 39 27H28.23L32.13 23.13C32.4112 22.8511 32.6344 22.5193 32.7867 22.1537C32.939 21.7881 33.0174 21.396 33.0174 21C33.0174 20.604 32.939 20.2118 32.7867 19.8463C32.6344 19.4807 32.4112 19.1489 32.13 18.87C31.8511 18.5888 31.5193 18.3656 31.1537 18.2133C30.7881 18.061 30.396 17.9826 30 17.9826C29.604 17.9826 29.2118 18.061 28.8463 18.2133C28.4807 18.3656 28.1489 18.5888 27.87 18.87L18.87 27.87ZM0 30C0 35.9334 1.75947 41.7336 5.05591 46.6671C8.35235 51.6006 13.0377 55.4457 18.5195 57.7164C24.0013 59.987 30.0333 60.5811 35.8527 59.4235C41.6721 58.266 47.0176 55.4088 51.2132 51.2132C55.4088 47.0176 58.266 41.6721 59.4235 35.8527C60.5811 30.0333 59.987 24.0013 57.7164 18.5195C55.4457 13.0377 51.6006 8.35235 46.6671 5.05591C41.7336 1.75947 35.9334 0 30 0C26.0603 0 22.1593 0.775973 18.5195 2.28361C14.8797 3.79125 11.5726 6.00104 8.78679 8.78679C3.1607 14.4129 0 22.0435 0 30ZM54 30C54 34.7467 52.5924 39.3869 49.9553 43.3337C47.3181 47.2804 43.5698 50.3566 39.1844 52.1731C34.799 53.9896 29.9734 54.4649 25.3178 53.5388C20.6623 52.6128 16.3859 50.327 13.0294 46.9706C9.67297 43.6141 7.3872 39.3377 6.46115 34.6822C5.53511 30.0266 6.01039 25.201 7.82689 20.8156C9.64339 16.4302 12.7195 12.6819 16.6663 10.0447C20.6131 7.40757 25.2532 6 30 6C36.3652 6 42.4697 8.52856 46.9706 13.0294C51.4714 17.5303 54 23.6348 54 30Z" fill="#currentColor" />
                </svg>
                <div className="list-feedback">
                    <FeedbackItem feedback={listFeedback[currentPage].feedback} author={listFeedback[currentPage].author} />
                    {useWindowWidth() > 1200 && (
                        <FeedbackItem feedback={listFeedback[(currentPage+1+listFeedback.length) % listFeedback.length].feedback} author={listFeedback[(currentPage+1+listFeedback.length) % listFeedback.length].author} />
                    )}
                </div>
                <svg className="arrow right" width="60" height="60" viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg" tabIndex={0} onClick={onClickNext}>
                    <path d="M18.87 27.87C18.5969 28.1553 18.3828 28.4917 18.24 28.86C17.9399 29.5904 17.9399 30.4096 18.24 31.14C18.3828 31.5082 18.5969 31.8447 18.87 32.13L27.87 41.13C28.4349 41.6949 29.2011 42.0123 30 42.0123C30.7989 42.0123 31.5651 41.6949 32.13 41.13C32.6949 40.5651 33.0123 39.7989 33.0123 39C33.0123 38.2011 32.6949 37.4349 32.13 36.87L28.23 33H39C39.7956 33 40.5587 32.6839 41.1213 32.1213C41.6839 31.5587 42 30.7956 42 30C42 29.2043 41.6839 28.4413 41.1213 27.8787C40.5587 27.3161 39.7956 27 39 27H28.23L32.13 23.13C32.4112 22.8511 32.6344 22.5193 32.7867 22.1537C32.939 21.7881 33.0174 21.396 33.0174 21C33.0174 20.604 32.939 20.2118 32.7867 19.8463C32.6344 19.4807 32.4112 19.1489 32.13 18.87C31.8511 18.5888 31.5193 18.3656 31.1537 18.2133C30.7881 18.061 30.396 17.9826 30 17.9826C29.604 17.9826 29.2118 18.061 28.8463 18.2133C28.4807 18.3656 28.1489 18.5888 27.87 18.87L18.87 27.87ZM0 30C0 35.9334 1.75947 41.7336 5.05591 46.6671C8.35235 51.6006 13.0377 55.4457 18.5195 57.7164C24.0013 59.987 30.0333 60.5811 35.8527 59.4235C41.6721 58.266 47.0176 55.4088 51.2132 51.2132C55.4088 47.0176 58.266 41.6721 59.4235 35.8527C60.5811 30.0333 59.987 24.0013 57.7164 18.5195C55.4457 13.0377 51.6006 8.35235 46.6671 5.05591C41.7336 1.75947 35.9334 0 30 0C26.0603 0 22.1593 0.775973 18.5195 2.28361C14.8797 3.79125 11.5726 6.00104 8.78679 8.78679C3.1607 14.4129 0 22.0435 0 30ZM54 30C54 34.7467 52.5924 39.3869 49.9553 43.3337C47.3181 47.2804 43.5698 50.3566 39.1844 52.1731C34.799 53.9896 29.9734 54.4649 25.3178 53.5388C20.6623 52.6128 16.3859 50.327 13.0294 46.9706C9.67297 43.6141 7.3872 39.3377 6.46115 34.6822C5.53511 30.0266 6.01039 25.201 7.82689 20.8156C9.64339 16.4302 12.7195 12.6819 16.6663 10.0447C20.6131 7.40757 25.2532 6 30 6C36.3652 6 42.4697 8.52856 46.9706 13.0294C51.4714 17.5303 54 23.6348 54 30Z" fill="#currentColor" />
                </svg>
            </div>
        </div>
    )
}

export default Feedback